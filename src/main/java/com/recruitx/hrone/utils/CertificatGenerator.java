...
public class CertificatGenerator {
    ...
    // ✅ Retourne le PDF en bytes pour stockage DB
    public static byte[] genererBytes(String nomCandidat, String titreFormation,
                                      String niveau, String logoPath) {
        try {
            ...
            // Nous certifions que
            Font fontNormal = FontFactory.getFont(FontFactory.TIMES_ITALIC, 18, Color.DARK_GRAY);
            Paragraph certifie = new Paragraph("\nNous certifions que\n", fontNormal);
            certifie.setAlignment(Element.ALIGN_CENTER);
            document.add(certifie);
            ...
            Font fontFormation = FontFactory.getFont(FontFactory.TIMES_BOLD, 24,
                    new Color(212, 175, 55));
            Paragraph formation = new Paragraph("« " + titreFormation + " »", fontFormation);
            formation.setAlignment(Element.ALIGN_CENTER);
            document.add(formation);
            ...
            // Date
            String dateEmission = java.time.LocalDate.now().format(
                    java.time.format.DateTimeFormatter.ofPattern("dd MMMM yyyy",
                            java.util.Locale.FRENCH));
            Font fontDate = FontFactory.getFont(FontFactory.HELVETICA, 13, Color.GRAY);
            Paragraph date = new Paragraph("\nDélivré le " + dateEmission, fontDate);
            date.setAlignment(Element.ALIGN_CENTER);
            document.add(date);

            // Signature
            Font fontSign = FontFactory.getFont(FontFactory.TIMES_ITALIC, 14, Color.DARK_GRAY);
            Paragraph signature = new Paragraph(
                    "\n\n\n_______________________\nL'Agent RH", fontSign);
            signature.setAlignment(Element.ALIGN_RIGHT);
            signature.setIndentationRight(60f);
            ...
            document.close();
            System.out.println("✅ Certificat généré en mémoire");
            return baos.toByteArray();

        } catch (Exception e) {
            System.err.println("❌ Erreur génération certificat bytes: " + e.getMessage());
            return null;
        }
    }
}